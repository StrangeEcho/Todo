...
class TaskModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        msg = await chan.send(
            ...
            .set_footer(text="❌ Decline | 👷 Claim | ✅ Mark Finished")
        )
        for reaction in self._reactions:
            await msg.add_reaction(reaction)
        await interaction.followup.send(
            f"Finished adding task to Todolist", ephemeral=True
        )
...
class Meta(commands.Cog):
    ...
    async def ping(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                title="Measured Out Latency", color=discord.Color.blue()
            )
            .add_field(
                name="Websocket/Gateway",
                value="\n".join(
                    [
                        f"`Shard ID: {shard_id} | {round(latency * 1000)}ms`"
                        for shard_id, latency in self.bot.latencies
                    ]
                ),
            )
            .add_field(name="HTTP Protocol", value=f"`{edit_latency}ms`", inline=False)
        )
        await msg.delete()
    ...
    async def cleanup(self, ctx: commands.Context, limit: int = 15):
        ...
        msgs = await ctx.channel.purge(
            limit=limit, check=lambda x: x.author.id == self.bot.user.id
        )
    ...
    async def todo(
        self, inter: discord.Interaction, task_type: Literal["Map", "Scripts"]
    ):
        """Add a task to the todo list"""
        await inter.response.send_modal(
            TaskModal(self.bot.todo_channel, modal_typle=task_type)
        )
    ...
    async def taskrevise(
        self, inter: discord.Interaction, task_id: str, *, revision: str
    ):
        """Revise an open task"""
